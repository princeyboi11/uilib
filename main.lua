--[[
    ...
    Version: 3.0 - Complete Rebrand
    
    Features:
    - Modern neon aesthetic with glassmorphism effects
    - 5 stunning theme presets (Neon, Ember, Frost, Phantom, Aurora)
    - Enhanced toggles with glow animations
    - Tabbed interface system
    - Spectacular loading animations
    - Premium notification system
    
    API:
    - KnoxHub:CreateWindow(Config) - Create main window
    - KnoxHub:CreateLoadingScreen(Config) - Show loading animation
    - KnoxHub:SetTheme(ThemeName) - Switch themes
    - KnoxHub:Notify(Config) - Show notification
--]]
...
local KnoxHub = {
	...
	Windows = {},
	Scale = {
		Window = UDim2.new(0, 520, 0, 600),
		Mobile = UDim2.new(0, 460, 0, 400),
		TabOpen = 200,
		TabClose = 90,
		CornerRadius = 12,
	},
...
-- Knox Neon Theme (Default) - Vibrant cyan/purple cyberpunk aesthetic
KnoxHub.Colors = {
	-- Primary accent colors
	Highlight = Color3.fromRGB(0, 245, 255),           -- Vibrant Cyan
	Toggle = Color3.fromRGB(138, 43, 226),             -- Neon Purple
	Risky = Color3.fromRGB(255, 100, 100),             -- Soft Red Glow
	AccentGlow = Color3.fromRGB(0, 200, 255),          -- Glow color
	
	-- Background layers (deep space)
	BGDBColor = Color3.fromRGB(8, 8, 16),              -- Deep space black
	BlockColor = Color3.fromRGB(14, 14, 24),           -- Card background
	BlockBackground = Color3.fromRGB(20, 20, 35),      -- Panel background
	
	-- Interactive elements
	DropColor = Color3.fromRGB(25, 25, 42),            -- Dropdown/input bg
	MouseEnter = Color3.fromRGB(45, 45, 75),           -- Hover state
	
	-- Borders & lines
	StrokeColor = Color3.fromRGB(40, 40, 65),          -- Subtle borders
	HighStrokeColor = Color3.fromRGB(55, 55, 90),      -- Active borders
	LineColor = Color3.fromRGB(50, 50, 80),            -- Dividers
	
	-- Text colors
	SwitchColor = Color3.fromRGB(255, 255, 255),       -- Primary text
	TextMuted = Color3.fromRGB(140, 140, 170),         -- Secondary text
};
...
KnoxHub.DragBlacklist = {};
KnoxHub.IaDrag = false;
KnoxHub.LastDrag = tick();
KnoxHub.Flags = {};
...
function KnoxHub:OptimizeMode(v)
	KnoxHub.PerformanceMode = v;
end;

function KnoxHub:IsStudio()
	return RunService:IsStudio()	
end;

function KnoxHub:CustomIconHighlight()
	KnoxHub.CustomHighlightMode = true;
end;

function KnoxHub:_SetNilP(Ins: Instance , Parent: Instance)
	KnoxHub.WindowsNil = KnoxHub.WindowsNil or {};
	KnoxHub.NilFolder = KnoxHub.NilFolder or Instance.new('Folder');

	if not KnoxHub.WindowsNil[Ins] then
		local win = KnoxHub:_GetWindowFromElement(Ins);

		KnoxHub.WindowsNil[Ins] = win;
	end;

	Ins.Parent = Parent or KnoxHub.NilFolder;
end;

function KnoxHub:SetAllText(flags : {[string] : string})
	if not flags then -- reset to default
		for i,v in next , KnoxHub.Flags do
			if v.SetText then
				v:SetText(nil);
			end;
		end;
	...
	for i,v in next , flags do
		if KnoxHub.Flags[i] and KnoxHub.Flags[i].SetText then
			KnoxHub.Flags[i]:SetText(v);
		end;
	end;
end;

function KnoxHub:_GetIcon(name : string , font_aws) : string
	if KnoxHub.SecureMode then
		local AssetId;

		if font_aws then
			AssetId = KnoxHub.FontAwesome[name] or name;
		else
			AssetId = KnoxHub.Lucide['lucide-'..tostring(name)] or KnoxHub.Lucide[name] or KnoxHub.Lucide[tostring(name)] or KnoxHub.FontAwesome[name] or name;
		end;

		if AssetId and AssetId ~= nil then
			local asset = KnoxHub:CacheImage(AssetId);
	...
	if font_aws then
		return KnoxHub.FontAwesome[name] or name;
	end;

	return KnoxHub.Lucide['lucide-'..tostring(name)] or KnoxHub.Lucide[name] or KnoxHub.Lucide[tostring(name)] or KnoxHub.FontAwesome[name] or name;
end;

function KnoxHub:_RandomString() : string
...
function KnoxHub:_IsMouseOverFrame(Frame : Frame) : boolean
	if not Frame then
		return;
	end;
...
function KnoxHub:_Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function KnoxHub:_Animation(Self: Instance , Info: TweenInfo , Property :{[K] : V})
...
function KnoxHub:_Input(Frame : Frame , Callback : () -> ()) : TextButton
...
function KnoxHub:GetCalculatePosition(planePos: number, planeNormal: number, rayOrigin: number, rayDirection: number) : number
	local n = planeNormal;
...
function KnoxHub:_Blur(element : Frame , WindowRemote) : RBXScriptSignal
	if KnoxHub.SecureMode and not KnoxHub.SecurityConfig.BlurEnabled then
		return game.Changed:Connect(function() end);
	end;

	local Part = Instance.new('Part',KnoxHub.ArcylicParent);
	...
	Part.CanQuery = false;
	Part.CollisionGroup = KnoxHub:_RandomString();
	...
	DepthOfField.Name = KnoxHub:_RandomString();

	Part.Name = KnoxHub:_RandomString();
	...
	local UpdateFunction = function()
		...
		if IsWindowActive then
			...
			KnoxHub:_Animation(DepthOfField,TweenInfo.new(0.1),{
				NearIntensity = 1
			})

			KnoxHub:_Animation(Part,TweenInfo.new(0.1),{
				Transparency = 0.97,
		...
		if IsWindowActive then
			...
			local pos0 = KnoxHub:GetCalculatePosition(planeOrigin, planeNormal, ray0.Origin, ray0.Direction);
			local pos1 = KnoxHub:GetCalculatePosition(planeOrigin, planeNormal, ray1.Origin, ray1.Direction);
...
function KnoxHub:_AddDragBlacklist(Frame: Frame)
	local IsAdded = false;
	local BASE_TIME = 0.01;

	local SET_BLACKLIST = function(value)
		local index = table.find(KnoxHub.DragBlacklist , Frame);

		if value and not KnoxHub.IS_DRAG_MOVE then
			if not index then
				table.insert(KnoxHub.DragBlacklist,Frame);
			end;
		else
			if index then
				table.remove(KnoxHub.DragBlacklist,index);
			end;
		end;
	end;

	Frame.InputBegan:Connect(function(input)
		if KnoxHub:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(true)
		end;
	end);
	...
	UserInputService.InputChanged:Connect(function()
		if not KnoxHub:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(false);
		end
	end);
end;

function KnoxHub:_GetWindowFromElement(Element)
	if KnoxHub.WindowsNil[Element] then
		return KnoxHub.WindowsNil[Element];
	end;

	for i,v : ScreenGui in next , KnoxHub.Windows do
		if v and Element:IsDescendantOf(v) then
			return v;
		end;
	end;

	for Frame,Window in next , KnoxHub.WindowsNil do
...
function KnoxHub.__SIGNAL(default)
...
function KnoxHub:_Hover(Frame: Frame , OnHover: () -> any?, Release: () -> any?)
	Frame.MouseEnter:Connect(OnHover);

	Frame.MouseLeave:Connect(Release);
end;

function KnoxHub.__CONFIG(config , default)
	config = config or {};
...
function KnoxHub:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local updateInput = function(input)
		...
		KnoxHub:_Animation(MoveFrame,Tween,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #KnoxHub.DragBlacklist <= 0 then 
			dragToggle = true
			...
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					dragToggle = false;
					KnoxHub.IS_DRAG_MOVE = false;
				end
			end)
		end

		if not KnoxHub.IsDrage and dragToggle then
			KnoxHub.LastDrag = tick();
		end;

		KnoxHub.IaDrag = dragToggle;
	end)

	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #KnoxHub.DragBlacklist <= 0 then
			if dragToggle then
				KnoxHub.IS_DRAG_MOVE = true;
				updateInput(input)
			else
				KnoxHub.IS_DRAG_MOVE = false;
			end
		else
			if #KnoxHub.DragBlacklist > 0 then
				dragToggle = false
				KnoxHub.IS_DRAG_MOVE = false;
			end
		end
...
function KnoxHub:_IsMobile()
	return UserInputService.TouchEnabled;
end;

function KnoxHub:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal)
	if Name == "Toggle" then
		...
		Toggle.Name = KnoxHub:_RandomString()
		Toggle.Parent = LinkValues
		Toggle.BackgroundColor3 = KnoxHub.Colors.DropColor
		Toggle.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Toggle.BorderSizePixel = 0
		Toggle.Size = UDim2.new(0, 36, 0, 20) -- Larger, more satisfying toggle
		Toggle.ZIndex = GlobalBlock.ZIndex + 1
		Toggle.LayoutOrder = -#LinkValues:GetChildren();

		table.insert(KnoxHub.Elements.DropColor , {
			Element = Toggle,
		...
		UIStroke.Color = KnoxHub.Colors.HighStrokeColor
		UIStroke.Parent = Toggle

		table.insert(KnoxHub.Elements.HighStrokeColor,{
			Element = UIStroke,
			Property = "Color"
		})
		...
		ToggleGlow.Name = KnoxHub:_RandomString()
		ToggleGlow.Parent = Toggle
		ToggleGlow.BackgroundColor3 = KnoxHub.Colors.Toggle
		ToggleGlow.BackgroundTransparency = 1
		ToggleGlow.BorderSizePixel = 0
		ToggleGlow.AnchorPoint = Vector2.new(0.5, 0.5)
		ToggleGlow.Position = UDim2.new(0.5, 0, 0.5, 0)
		ToggleGlow.Size = UDim2.new(1.3, 0, 1.5, 0)
		ToggleGlow.ZIndex = GlobalBlock.ZIndex
		local GlowCorner = Instance.new("UICorner")
		GlowCorner.CornerRadius = UDim.new(1, 0)
		GlowCorner.Parent = ToggleGlow

		ToggleValue.Name = KnoxHub:_RandomString()
		ToggleValue.Parent = Toggle
		ToggleValue.AnchorPoint = Vector2.new(0.5, 0.5)
		ToggleValue.BackgroundColor3 = KnoxHub.Colors.SwitchColor
		...
		local ToggleElement = function(bool,noChange)
			...
			-- Enhanced elastic animation for satisfying feel
			local bounceTween = TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out);

			if bool then
				Toggle:SetAttribute('Enabled',true);

				KnoxHub:_Animation(ToggleValue,bounceTween,{
					Position = UDim2.new(0.75, 0, 0.5, 0)
				})

				KnoxHub:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = KnoxHub.Colors.Toggle
				})

				-- Enable glow effect
				if KnoxHub.GlowEnabled then
					KnoxHub:_Animation(ToggleGlow,TweenInfo.new(0.4),{
				...
				KnoxHub:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = KnoxHub.Colors.DropColor
				})

				-- Disable glow effect
				KnoxHub:_Animation(ToggleGlow,TweenInfo.new(0.3),{
					BackgroundTransparency = 1
				})
			end;
		end;

		local Input = KnoxHub:_Input(Toggle);

		KnoxHub:_Hover(Input , function()
			if not Default then
				KnoxHub:_Animation(ToggleValue,rep.Tween,{
		...
		local ToggleUI = function(bool)
			if bool then
				...
				KnoxHub:_Animation(UIStroke,rep.Tween,{
					Transparency = 0
				})
			else
	...
	elseif Name == "ColorPicker" then
		...
		ColorPicker.Name = KnoxHub:_RandomString()
		...
		ColorFrame.Name = KnoxHub:_RandomString()
		...
		Signal:Connect(function(bool)
			if bool then
				KnoxHub:_Animation(ColorFrame,TweenInfo.new(0.15),{
					BackgroundTransparency = 0,
				})

				KnoxHub:_Animation(UIStroke,TweenInfo.new(0.15),{
					Transparency = 0,
				})
			else
				KnoxHub:_Animation(ColorFrame,TweenInfo.new(0.15),{
		...
		KnoxHub:_Hover(ColorPicker, function()
			if Signal:GetValue() then
				KnoxHub:_Animation(UIScale,TweenInfo.new(0.35),{
					Scale = 1.2
				})
			end;
		end , function()
	...
	elseif Name == "Keybind" then
		...
		Keybind.Name = KnoxHub:_RandomString()
		Keybind.Parent = LinkValues
		Keybind.BackgroundColor3 = KnoxHub.Colors.DropColor
		...
		TextLabel.TextColor3 = KnoxHub.Colors.SwitchColor
		TextLabel.TextSize = 12.000
		TextLabel.TextTransparency = 0.200

		table.insert(KnoxHub.Elements.SwitchColor,{
			Element = TextLabel,
			Property = "TextColor3"
		});
		...
		local Update = function()
			...
			KnoxHub:_Animation(Keybind,TweenInfo.new(0.1),{
		...
		local ToggleUI = function(bool)
			if bool then
				KnoxHub:_Animation(Keybind,rep.Tween,{
				...
				KnoxHub:_Animation(TextLabel,rep.Tween,{
	...
	elseif Name == "Helper" then
		...
		InfoButton.Name = KnoxHub:_RandomString()
		...
		InfoButton.Image = KnoxHub:CacheImage("rbxassetid://10723415903")
		InfoButton.ImageTransparency = 0.500

		UICorner.CornerRadius = UDim.new(1, 0)
		UICorner.Parent = InfoButton

		BlockText.Name = KnoxHub:_RandomString()
		BlockText.Parent = InfoButton
		BlockText.AnchorPoint = Vector2.new(0, 0)
		BlockText.BackgroundColor3 = KnoxHub.Colors.BlockColor

		table.insert(KnoxHub.Elements.BlockColor , {
			Element = BlockText,
			Property = "BackgroundColor3"
		});
		...
		BlockText.Text = " "
		BlockText.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		table.insert(KnoxHub.Elements.StrokeColor,{
			Element = UIStroke,
			Property = "Color"
		});
		...
		Signal:Connect(function(bool)
			if bool then
				KnoxHub:_Animation(InfoButton,TweenInfo.new(0.15),{
		...
		KnoxHub:_Hover(InfoButton, function()
			if Signal:GetValue() then
				KnoxHub:_Animation(InfoButton,TweenInfo.new(0.15),{
	...
	elseif Name == "Option" then
		...
		OptionButton.Name = KnoxHub:_RandomString()
		...
		OptionButton.Image = KnoxHub:CacheImage("rbxassetid://14007344336")
		...
		Signal:Connect(function(bool)
			if bool then
				KnoxHub:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.500
				})
			else
				KnoxHub:_Animation(OptionButton,TweenInfo.new(0.15),{
		...
		KnoxHub:_Hover(OptionButton, function()
			if Signal:GetValue() then
				KnoxHub:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.1
				})
...
function KnoxHub:_CreateBlock(Signal)
	...
	if KnoxHub:_IsMobile() then
		KnoxHub:_AddDragBlacklist(GlobalBlock);
	end;

	GlobalBlock.Name = KnoxHub:_RandomString()
	...
	LinkValues.Name = KnoxHub:_RandomString()
	...
	BlockLine.Name = KnoxHub:_RandomString()
	BlockLine.Parent = GlobalBlock
	BlockLine.AnchorPoint = Vector2.new(0.5, 1)
	BlockLine.BackgroundColor3 = KnoxHub.Colors.LineColor
	...
	table.insert(KnoxHub.Elements.LineColor,{
		Element = BlockLine,
		Property = "BackgroundColor3"
	});
	...
	function rep:SetTextColor(Color)
		local oldIndex = table.find(KnoxHub.Elements.SwitchColor , BlockText);

		table.remove(KnoxHub.Elements.SwitchColor , oldIndex);

		BlockText.TextColor3 = Color;

		table.insert(KnoxHub.Elements.Risky , {
			Element = BlockText,
			Property = 'TextColor3'
		});
	...
	function rep:SetTransparency(num)
		...
		KnoxHub:_Animation(BlockText,TweenInfo.new(0.3),{
	...
	function rep:SetVisible(bool)
		if bool then
			KnoxHub:_Animation(BlockText,rep.Tween,{
				TextTransparency = rep.TextTransparency
			});

			KnoxHub:_Animation(BlockLine,rep.Tween,{
	...
	function rep:AddLink(Name , Default)
		return KnoxHub:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal);
	end;

	return rep;
end;

KnoxHub.Hash = function(str: string)
...
function KnoxHub:CacheImage(id: string) : string
	if not KnoxHub.SecureMode or not id or not id:byte() then
		return id or "";
	end;

	assert(KnoxHub.SecureMode , "please use KnoxHub:Security(< string >) before cache image")
	assert(KnoxHub.CacheDirectory , "please use KnoxHub:Security(< string >) before cache image")
	...
	local Hash = KnoxHub.Hash(id);

	local cache_path = string.format("%s/cache-%s.png" ,KnoxHub.CacheDirectory , Hash);

	if isfile(cache_path) then
		return (getcustomasset or getsynasset or function() return ''; end)(cache_path);
	end;

	local imgSize = KnoxHub.SecurityConfig.ImageScale;
...
function KnoxHub:Security(directory: string,Config: SecurityConfig) -- Security Mode
	directory = directory or "KnoxHub-Cache";

	if not isfolder(directory) then
		makefolder(directory);
	end;

	KnoxHub.SecureMode = true;

	KnoxHub.SecurityConfig = Config or {};

	KnoxHub.CacheDirectory = directory;
end;

function KnoxHub:_AddColorPickerPanel(Button: ImageButton , Callback: (Color: Color3) -> any?)
	local Window = KnoxHub:_GetWindowFromElement(Button);
	...
	ColorPickerWindow.Name = KnoxHub:_RandomString()
	ColorPickerWindow.Parent = Window
	ColorPickerWindow.BackgroundColor3 = KnoxHub.Colors.BlockBackground
	...
	table.insert(KnoxHub.Elements.BlockBackground,{
		Element = ColorPickerWindow,
		Property = "BackgroundColor3"
	});

	KnoxHub:_AddDragBlacklist(ColorPickerWindow)
	...
	ColorPickBox.Name = KnoxHub:_RandomString()
	...
	ColorPickBox.Image = KnoxHub:CacheImage("http://www.roblox.com/asset/?id=112554223509763");

	MouseMovement.Name = KnoxHub:_RandomString()
	...
	MouseMovement.Image = KnoxHub:CacheImage("rbxassetid://4805639000")
	...
	ColorRedGreenBlue.Name = KnoxHub:_RandomString()
	...
	ColorRGBSlide.Name = KnoxHub:_RandomString()
	...
	Left.Name = KnoxHub:_RandomString()
	...
	Right.Name = KnoxHub:_RandomString()
	...
	ColorOpc.Name = KnoxHub:_RandomString()
	...
	ColorOptSlide.Name = KnoxHub:_RandomString()
	...
	Left_2.Name = KnoxHub:_RandomString()
	...
	Right_2.Name = KnoxHub:_RandomString()
	...
	TransparentImage.Name = KnoxHub:_RandomString()
	...
	TransparentImage.Image = KnoxHub:CacheImage("rbxassetid://6198493000")
	...
	HexFrame.Name = KnoxHub:_RandomString()
	HexFrame.Parent = ColorPickerWindow
	HexFrame.AnchorPoint = Vector2.new(0.5, 1)
	HexFrame.BackgroundColor3 = KnoxHub.Colors.BlockColor
	...
	UIStroke_8.Color = KnoxHub.Colors.HighStrokeColor
	...
	KnoxHub:_AddPropertyEvent(ColorPickerWindow,function(v)
		ColorPickerWindow.Visible = v;

		if KnoxHub.PerformanceMode then
			if ColorPickerWindow.Visible then
				KnoxHub:_SetNilP(ColorPickerWindow , Window);
			else
				KnoxHub:_SetNilP(ColorPickerWindow , nil);
			end;
		else
			KnoxHub:_SetNilP(ColorPickerWindow , Window);
		end;
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			KnoxHub:_Animation(ColorPickerWindow,Tween2,{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 175, 0, 200)
			});

			KnoxHub:_Animation(ColorPickerWindow,Tween,{
				Position = MainPosition,
			});

			KnoxHub:_Animation(UIStroke_8,Tween,{
				Transparency = 0
			});

			KnoxHub:_Animation(UIStroke_7,Tween,{
				Transparency = 0.5
			});

			KnoxHub:_Animation(UIStroke_6,Tween,{
				Transparency = 0
			});

			KnoxHub:_Animation(UIStroke_5,Tween,{
				Transparency = 0
			});

			KnoxHub:_Animation(UIStroke_4,Tween,{
				Transparency = 0
			});

			KnoxHub:_Animation(UIStroke_3,Tween,{
				Transparency = 0
			});

			KnoxHub:_Animation(UIStroke_2,Tween,{
				Transparency = 0
			});

			KnoxHub:_Animation(UIStroke,Tween,{
				Transparency = 0
			});

			KnoxHub:_Animation(ColorPickBox,Tween,{
				BackgroundTransparency = 0,
				ImageTransparency = 0
			});

			KnoxHub:_Animation(MouseMovement,Tween,{
				ImageTransparency = 0
			});

			KnoxHub:_Animation(ColorOpc,Tween,{
				BackgroundTransparency = 0
			});

			KnoxHub:_Animation(TransparentImage,Tween,{
				ImageTransparency = 0
			});

			KnoxHub:_Animation(Left,Tween,{
				BackgroundTransparency = 0
			});

			KnoxHub:_Animation(Left_2,Tween,{
				BackgroundTransparency = 0
			});

			KnoxHub:_Animation(Right,Tween,{
				BackgroundTransparency = 0
			});

			KnoxHub:_Animation(Right_2,Tween,{
				BackgroundTransparency = 0
			});

			KnoxHub:_Animation(ColorRedGreenBlue,Tween,{
				BackgroundTransparency = 0
			});

			KnoxHub:_Animation(HexFrame,Tween,{
				BackgroundTransparency = 0
			});

			KnoxHub:_Animation(TextLabel,Tween,{
				TextTransparency = 0
			});
		else
			KnoxHub:_Animation(UIStroke_8,Tween,{
	...
	function Args:Update()
		...
		KnoxHub:_Animation(ColorPickBox,TweenInfo.new(0.2),{
			BackgroundColor3 = Color3.fromHSV(H , 1 , 1)
		});

		KnoxHub:_Animation(ColorOpc,TweenInfo.new(0.2),{
			BackgroundColor3 = RealColor
		});

		KnoxHub:_Animation(MouseMovement,TweenInfo.new(0.2),{
			Position = UDim2.fromScale(S , 1 - V)
		});

		KnoxHub:_Animation(ColorOptSlide,TweenInfo.new(0.2),{
			Position = UDim2.new(Transparency ,0 , 0.5 ,0)
		});

		KnoxHub:_Animation(ColorRGBSlide,TweenInfo.new(0.2),{
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not KnoxHub:_IsMouseOverFrame(ColorPickerWindow) then
				ToggleUI(false);
			end;
		end;
	end)
...
function KnoxHub:_DrawKeybinds(Window: ScreenGui)
	if KnoxHub.__KEYBINDS_CACHE then
		return KnoxHub.__KEYBINDS_CACHE;
	end;
	...
	Keybinds.Name = KnoxHub:_RandomString()
	Keybinds.Parent = Window
	Keybinds.BackgroundColor3 = KnoxHub.Colors.BGDBColor;
	...
	table.insert(KnoxHub.Elements.BGDBColor,{
		Element = Keybinds,
		Property = 'BackgroundColor3'
	});
	...
	IconFrame.Name = KnoxHub:_RandomString()
	IconFrame.Parent = Keybinds
	IconFrame.AnchorPoint = Vector2.new(1, 0.5)
	IconFrame.BackgroundColor3 = KnoxHub.Colors.BGDBColor
	...
	Frame.BackgroundColor3 = KnoxHub.Colors.Highlight;
	...
	table.insert(KnoxHub.Elements.Highlight,{
		Element = Frame,
		Property = 'BackgroundColor3'
	});

	Icon.Name = KnoxHub:_RandomString()
	...
	Icon.ZIndex = 159
	Icon.Image = KnoxHub:CacheImage("rbxassetid://10723416765");

	HeaderFrame.Name = KnoxHub:_RandomString()
	...
	HeadLabel.Name = KnoxHub:_RandomString()
	...
	HeadLabel.TextColor3 = KnoxHub.Colors.SwitchColor
	...
	MainFrame.Name = KnoxHub:_RandomString()
	...
	MovingFrame.Name = KnoxHub:_RandomString()
	...
	KnoxHub:Drag(MovingFrame,Keybinds,0.1);
	...
	Ref.THREAD = task.spawn(function()
		while true do task.wait()
			KnoxHub:_Animation(MainFrame,TweenInfo.new(0.4),{
				Size = UDim2.new(1, 30, 1, UIListLayout.AbsoluteContentSize.Y + 1)
			});

			if UIListLayout.AbsoluteContentSize.Y > 1 then
				KnoxHub:_Animation(IconFrame,TweenInfo.new(0.25),{
					BackgroundTransparency = 0.3
				})

				KnoxHub:_Animation(Frame,TweenInfo.new(0.25),{
					BackgroundTransparency = 0
				})

				KnoxHub:_Animation(HeadLabel,TweenInfo.new(0.25),{
					TextTransparency = 0
				})

				KnoxHub:_Animation(Icon,TweenInfo.new(0.25),{
					ImageTransparency = 0
				});
				...
				KnoxHub:_Animation(Keybinds,TweenInfo.new(0.25),{
			...
			if KnoxHub.PerformanceMode then
				if Keybinds.Visible then
					KnoxHub:_SetNilP(Keybinds , Window);
				else
					KnoxHub:_SetNilP(Keybinds , nil);
				end;
			else
				KnoxHub:_SetNilP(Keybinds , Window);
			end;
	...
	function Ref:AddFrame()
		...
		Keyholder.Name = KnoxHub:_RandomString()
		Keyholder.BackgroundColor3 = KnoxHub.Colors.BGDBColor
		...
		Label.Name = KnoxHub:_RandomString()
		...
		Label.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		Line.Name = KnoxHub:_RandomString()
		Line.Parent = Keyholder
		Line.AnchorPoint = Vector2.new(1, 0.5)
		Line.BackgroundColor3 = KnoxHub.Colors.BGDBColor
		...
		TypeLabel.Name = KnoxHub:_RandomString()
		...
		TypeLabel.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		function frame_ref:SetVisible(v)
			if v then
				KnoxHub:_Animation(Keyholder,TweenInfo.new(0.1),{
					BackgroundTransparency = 0.600,
					Size = UDim2.new(1, 0, 0, 28)
				});

				KnoxHub:_Animation(Label,TweenInfo.new(0.15),{
					TextTransparency = 0.100
				});

				KnoxHub:_Animation(Line,TweenInfo.new(0.15),{
					BackgroundTransparency = 0
				});

				KnoxHub:_Animation(TypeLabel,TweenInfo.new(0.15),{
					TextTransparency = 0
				});
			else
				KnoxHub:_Animation(Keyholder,TweenInfo.new(0.1),{
	...
	KnoxHub.__KEYBINDS_CACHE = Ref;

	return Ref;
end;

function KnoxHub:_KeybindHandler(Parent: Frame , ObjectType: string , ElementAPI: Toggle & Slider , Signal , Zindex: number , ElementCFG: Slider)
	local Window = KnoxHub:_GetWindowFromElement(Parent);
	local KB_Signal = KnoxHub.__SIGNAL(false);
	local SubIndex = math.random(40,100);
	local KeybindInd = KnoxHub:_DrawKeybinds(Window);
	...
	KeybindHandler.Name = KnoxHub:_RandomString()
	KeybindHandler.Parent = Window
	KeybindHandler.BackgroundColor3 = KnoxHub.Colors.BlockBackground
	...
	ElementObjs.Name = KnoxHub:_RandomString()
	...
	local refreshPF = function()
		if KnoxHub.PerformanceMode then
			if KeybindHandler.Size.Y.Offset > 1 then
				KnoxHub:_SetNilP(KeybindHandler , Window);
			else
				KnoxHub:_SetNilP(KeybindHandler , nil);
			end;
		else
			KnoxHub:_SetNilP(KeybindHandler , Window);
		end;
	...
	local ToggleUI = function(bool)
		if bool then
			...
			KnoxHub:_Animation(KeybindHandler,TweenInfo.new(0.25),{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 225, 0, UIListLayout.AbsoluteContentSize.Y + 5)
			});

			KnoxHub:_Animation(UIStroke,TweenInfo.new(0.3),{
				Transparency = 0
			});
		else
			KnoxHub:_Animation(KeybindHandler,TweenInfo.new(0.3),{
				BackgroundTransparency = 1,
	...
	local ElementAPIs = KnoxHub:_LoadElement(ElementObjs , true , KB_Signal , true);
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.MouseButton2 or Input.UserInputType == Enum.UserInputType.Touch then
			if not KnoxHub:_IsMouseOverFrame(Parent) and not KnoxHub:_IsMouseOverFrame(KeybindHandler) then
...
function KnoxHub:_LoadOption(Value , TabSignal)
	...
	function Args:AddKeybind(Config: MiniKeybind)
		...
		KnoxHub:_Input(Keybind.Root,function()
			if IsBinding then
				return;
			end;
		...
		if Config.Flag then
			KnoxHub.Flags[Config.Flag] = Args;
		end;
	...
	function Args:AddHelper(Config: Helper)
		...
		Helper.UIStroke:GetPropertyChangedSignal('Transparency'):Connect(function()
			...
			if KnoxHub.PerformanceMode then
				if Helper.Text.Visible then
					KnoxHub:_SetNilP(Helper.Text , Window);
				else
					KnoxHub:_SetNilP(Helper.Text , nil);
				end;
			else
				KnoxHub:_SetNilP(Helper.Text , Window);
			end;
		...
		local Update = function()
			...
			KnoxHub:_Animation(Helper.Text , TweenInfo.new(0.15), {
		...
		local Release = function()
			...
			KnoxHub:_Animation(Helper.UIStroke,TweenInfo.new(0.15),{
				Transparency = 1
			});
		end;
		...
		KnoxHub:_Hover(Button,  Hold, Release);
	...
	function Args:AddColorPicker(Config: MiniColorPicker)
		...
		local Button = KnoxHub:_Input(ColorPicker);

		local ColorPicker = KnoxHub:_AddColorPickerPanel(Button,function(color,opc)
	...
	function Args:AddOption()
		...
		local Signal = KnoxHub.__SIGNAL(false);
		...
		local ToggleUI = function(bool)
			...
			if bool then
				...
				KnoxHub:_Animation(ExtractElement , Tween , {
					Position = MainPosition,
					BackgroundTransparency = 0,
		...
		KnoxHub:_AddPropertyEvent(ExtractElement,function(bool)
			ExtractElement.Visible = bool;

			if KnoxHub.PerformanceMode then
				if ExtractElement.Visible then
					KnoxHub:_SetNilP(ExtractElement , Window);
				else
					KnoxHub:_SetNilP(ExtractElement , nil);
				end;
			else
				KnoxHub:_SetNilP(ExtractElement , Window);
			end;
		...
		KnoxHub:_AddDragBlacklist(ExtractElement);

		ExtractElement.Name = KnoxHub:_RandomString()
		ExtractElement.Parent = Window
		ExtractElement.BackgroundColor3 = KnoxHub.Colors.BlockBackground
		...
		Elements.Name = KnoxHub:_RandomString()
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if Toggl and not KnoxHub:_IsMouseOverFrame(ExtractElement) and not KnoxHub:_IsMouseOverFrame(Element) then
					ToggleUI(false);
				end;
			end
		end)		

		return KnoxHub:_LoadElement(Elements , true , Signal)
	end;

	return Args;
end;

function KnoxHub:_LoadDropdown(BaseParent: TextButton , Callback: () -> any)
	local Window = KnoxHub:_GetWindowFromElement(BaseParent);
	...
	local UIListLayout = Instance.new("UIListLayout")
	local ToggleDb = KnoxHub.__SIGNAL(false);
	local EventOut = KnoxHub.__SIGNAL(0);

	DropdownWindow.Name = KnoxHub:_RandomString()
	DropdownWindow.Parent = Window
	DropdownWindow.BackgroundColor3 = KnoxHub.Colors.BlockBackground
	...
	KnoxHub:_AddDragBlacklist(DropdownWindow);
	KnoxHub:_AddPropertyEvent(DropdownWindow,function(v)
		DropdownWindow.Visible = v;

		if KnoxHub.PerformanceMode then
			if DropdownWindow.Visible then
				KnoxHub:_SetNilP(DropdownWindow , Window);
			else
				KnoxHub:_SetNilP(DropdownWindow , nil);
			end;
		else
			KnoxHub:_SetNilP(DropdownWindow , Window);
		end;
	...
	local DrawButton = function()
		...
		DropdownItem.Name = KnoxHub:_RandomString()
	...
	local UpdateDropdown = function()
		...
		for i,v in next , Values do
			...
			table.insert(__signals , ToggleDb:Connect(function(bool)
				if bool then
					KnoxHub:_Animation(bth.BlockText,TweenInfo.new(0.2),{
						TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
					});

					KnoxHub:_Animation(bth.BlockLine,TweenInfo.new(0.2),{
						BackgroundTransparency = 0
					});
				else
					KnoxHub:_Animation(bth.BlockText,TweenInfo.new(0.2),{
			...
			if IsMulti then
				...
				KnoxHub:_Input(bth.DropdownItem,function()
				...
				KnoxHub:_Input(bth.DropdownItem,function()
					if DataFrame then
						KnoxHub:_Animation(DataFrame.BlockText,TweenInfo.new(0.2),{
							TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
						});
					end;
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not KnoxHub:_IsMouseOverFrame(DropdownWindow) then
				ToggleUI(false);
			end;
		end;
	end);
...
function KnoxHub:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
	...
	function Args:AddToggle(Config : Toggle)
		...
		local Block = KnoxHub:_CreateBlock(Signal);
		...
		if Config.Risky then
			Block:SetTextColor(KnoxHub.Colors.Risky);
		end;
		...
		Args.Link = KnoxHub:_LoadOption(Block);
		...
		if not DisableStackKeybind then
			local AutoKeybind = KnoxHub:_KeybindHandler(Block.Root , "Toggle" , Args , Signal , Zindex , Config);
	...
	function Args:AddButton(Config: Button)
		...
		if KnoxHub:_IsMobile() then
			KnoxHub:_AddDragBlacklist(Button);
		end;

		Button.Name = KnoxHub:_RandomString()
		...
		KnoxHub:_Hover(Frame,function()
			if Signal:GetValue() then
				KnoxHub:_Animation(Frame,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				})
			end;
		end,function()
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				KnoxHub:_Animation(BlockLine, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.500
				});

				KnoxHub:_Animation(Frame, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.1
				});

				KnoxHub:_Animation(UIStroke, TweenInfo.new(0.35),{
					Transparency = 0
				});

				KnoxHub:_Animation(TextLabel, TweenInfo.new(0.35),{
	...
	function Args:AddSlider(Config: Slider)
		...
		KnoxHub:_AddDragBlacklist(Slider);

		Slider.Name = KnoxHub:_RandomString()
		...
		SliderBar.Name = KnoxHub:_RandomString()
		SliderBar.Parent = Slider
		SliderBar.AnchorPoint = Vector2.new(0.5, 1)
		SliderBar.BackgroundColor3 = KnoxHub.Colors.DropColor
		...
		SliderInput.Name = KnoxHub:_RandomString()
		SliderInput.Parent = SliderBar
		SliderInput.AnchorPoint = Vector2.new(0, 0.5)
		SliderInput.BackgroundColor3 = KnoxHub.Colors.Highlight
		...
		Frame.BackgroundColor3 = KnoxHub.Colors.SwitchColor
		Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Frame.BorderSizePixel = 0
		Frame.Position = UDim2.new(1, 5, 0.5, 0)
		...
		ValueText.Name = KnoxHub:_RandomString()
		...
		ValueText.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		KnoxHub:_Hover(SliderBar,function()
			if Signal:GetValue() then
				KnoxHub:_Animation(ValueText,TweenInfo.new(0.2),{
		...
		local Update = function(Input)
			...
			local Value = KnoxHub:_Rounding(Main,Config.Round);
			...
			SliderBar.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not KnoxHub:_IsMouseOverFrame(SliderBar) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		function Args:SetValue(Value)
			...
			KnoxHub:_Animation(SliderInput, TweenInfo.new(0.35),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				KnoxHub:_Animation(ValueText,Tween,{
					TextTransparency = 0.750
				})

				KnoxHub:_Animation(Frame,Tween,{
					BackgroundTransparency = 0
				})

				KnoxHub:_Animation(SliderInput,Tween,{
					BackgroundTransparency = 0
				})
				...
				KnoxHub:_Animation(SliderBar,Tween,{
					BackgroundTransparency = 0
				})

				KnoxHub:_Animation(BlockLine,Tween,{
					BackgroundTransparency = 0.5
				})

				KnoxHub:_Animation(BlockText,Tween,{
		...
		if not DisableStackKeybind then
			local AutoKeybind = KnoxHub:_KeybindHandler(Slider , "Number" , Args , Signal , Zindex , Config);
	...
	function Args:AddParagraph(Config: Paragraph) -- request by Neptune
		...
		if KnoxHub:_IsMobile() then
			KnoxHub:_AddDragBlacklist(Paragraph);
		end;

		Paragraph.Name = KnoxHub:_RandomString()
		...
		DescriptionText.Name = KnoxHub:_RandomString()
		...
		DescriptionText.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		local UpdateScale = function()
			...
			if not DescriptionText.Text:byte() then
				...
				KnoxHub:_Animation(Paragraph,TweenInfo.new(0.15),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				KnoxHub:_Animation(BlockText,TweenInfo.new(0.2),{
					TextTransparency = 0.300
				});

				KnoxHub:_Animation(DescriptionText,TweenInfo.new(0.2),{
					TextTransparency = 0.500
				});

				KnoxHub:_Animation(BlockLine,TweenInfo.new(0.2),{
					BackgroundTransparency = 0.500
				});
			else
				KnoxHub:_Animation(BlockText,TweenInfo.new(0.2),{
	...
	function Args:AddTextBox(Config: TextBoxConfig)
		...
		if KnoxHub:_IsMobile() then
			KnoxHub:_AddDragBlacklist(TextBox);
		end;

		TextBox.Name = KnoxHub:_RandomString()
		...
		LinkValues.AnchorPoint = Vector2.new(1, 0.540000021)
		LinkValues.BackgroundColor3 = KnoxHub.Colors.DropColor
		LinkValues.BorderColor3 = Color3.fromRGB(0, 0, 0)
		LinkValues.BorderSizePixel = 0
		LinkValues.Position = UDim2.new(1, -12, 0.5, 0)
		...
		TextBox_2.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		local Update = function()
			...
			local xp = pcall(function()
				KnoxHub:_Animation(LinkValues,TweenInfo.new(0.25),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				KnoxHub:_Animation(LinkValues,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
	...
	function Args:AddDropdown(Config : Dropdown)
		...
		Dropdown.Name = KnoxHub:_RandomString()
		...
		ValueItems.Name = KnoxHub:_RandomString()
		ValueItems.Parent = Dropdown
		ValueItems.AnchorPoint = Vector2.new(0.5, 1)
		ValueItems.BackgroundColor3 = KnoxHub.Colors.DropColor
		...
		MainButton.Name = KnoxHub:_RandomString()
		...
		MainButton.Image = KnoxHub:CacheImage("rbxassetid://109535175596957")

		KnoxHub:_Hover(ValueItems,function()
			KnoxHub:_Animation(ValueItems,TweenInfo.new(0.3),{
				BackgroundColor3 = KnoxHub.Colors.MouseEnter
			});
		end,function()
			KnoxHub:_Animation(ValueItems,TweenInfo.new(0.3),{
				BackgroundColor3 = KnoxHub.Colors.DropColor
		...
		local repi;
		local Button = KnoxHub:_Input(ValueItems);

		repi = KnoxHub:_LoadDropdown(Button,function(value)
			Config.Default = value;
		...
		repi.EventOut:Connect(function(v)
			if v then
				KnoxHub:_Animation(MainButton,TweenInfo.new(0.2),{
					Rotation = -180
				})
			else
				KnoxHub:_Animation(MainButton,TweenInfo.new(0.2),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				KnoxHub:_Animation(ValueItems,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
				...
				KnoxHub:_Animation(ValueText,TweenInfo.new(0.32),{
					TextTransparency = 0
				});
		...
		Args.Link = KnoxHub:_LoadOption({
			AddLink = function(self ,Name , Default)
				return KnoxHub:_AddLinkValue(Name , Default , LinkValues , LinkValues , {
...
function KnoxHub:GetTheme()
	return KnoxHub.Colors;
end;

function KnoxHub:SetTheme(name)
	-- Knox Neon (Default) - Vibrant cyan/purple cyberpunk aesthetic
	if name == "Knox Neon" or name == "Default" then
		KnoxHub.Colors = {
			Highlight = Color3.fromRGB(0, 245, 255),
			Toggle = Color3.fromRGB(138, 43, 226),
			Risky = Color3.fromRGB(255, 100, 100),
			AccentGlow = Color3.fromRGB(0, 200, 255),
			BGDBColor = Color3.fromRGB(8, 8, 16),
			BlockColor = Color3.fromRGB(14, 14, 24),
			BlockBackground = Color3.fromRGB(20, 20, 35),
			DropColor = Color3.fromRGB(25, 25, 42),
			MouseEnter = Color3.fromRGB(45, 45, 75),
			StrokeColor = Color3.fromRGB(40, 40, 65),
			HighStrokeColor = Color3.fromRGB(55, 55, 90),
			LineColor = Color3.fromRGB(50, 50, 80),
			SwitchColor = Color3.fromRGB(255, 255, 255),
			TextMuted = Color3.fromRGB(140, 140, 170),
		};
	-- Knox Ember - Warm orange/red fire aesthetic
	elseif name == "Knox Ember" then
		KnoxHub.Colors = {
			Highlight = Color3.fromRGB(255, 120, 50),
			Toggle = Color3.fromRGB(255, 80, 80),
			Risky = Color3.fromRGB(255, 200, 50),
			AccentGlow = Color3.fromRGB(255, 100, 30),
			BGDBColor = Color3.fromRGB(12, 8, 8),
			BlockColor = Color3.fromRGB(20, 14, 12),
			BlockBackground = Color3.fromRGB(28, 20, 18),
			DropColor = Color3.fromRGB(35, 25, 22),
			MouseEnter = Color3.fromRGB(60, 40, 35),
			StrokeColor = Color3.fromRGB(50, 35, 30),
			HighStrokeColor = Color3.fromRGB(70, 50, 45),
			LineColor = Color3.fromRGB(65, 45, 40),
			SwitchColor = Color3.fromRGB(255, 255, 255),
			TextMuted = Color3.fromRGB(180, 150, 140),
		};
	-- Knox Frost - Cool ice blue/white minimal aesthetic
	elseif name == "Knox Frost" then
		KnoxHub.Colors = {
			Highlight = Color3.fromRGB(100, 200, 255),
			Toggle = Color3.fromRGB(150, 220, 255),
			Risky = Color3.fromRGB(255, 150, 150),
			AccentGlow = Color3.fromRGB(120, 210, 255),
			BGDBColor = Color3.fromRGB(15, 20, 28),
			BlockColor = Color3.fromRGB(22, 30, 42),
			BlockBackground = Color3.fromRGB(28, 38, 52),
			DropColor = Color3.fromRGB(35, 48, 65),
			MouseEnter = Color3.fromRGB(55, 75, 100),
			StrokeColor = Color3.fromRGB(50, 65, 85),
			HighStrokeColor = Color3.fromRGB(70, 90, 115),
			LineColor = Color3.fromRGB(60, 80, 105),
			SwitchColor = Color3.fromRGB(255, 255, 255),
			TextMuted = Color3.fromRGB(160, 180, 200),
		};
	-- Knox Phantom - Pure dark with subtle white/grey accents
	elseif name == "Knox Phantom" then
		KnoxHub.Colors = {
			Highlight = Color3.fromRGB(255, 255, 255),
			Toggle = Color3.fromRGB(180, 180, 180),
			Risky = Color3.fromRGB(255, 100, 100),
			AccentGlow = Color3.fromRGB(200, 200, 200),
			BGDBColor = Color3.fromRGB(5, 5, 5),
			BlockColor = Color3.fromRGB(12, 12, 12),
			BlockBackground = Color3.fromRGB(18, 18, 18),
			DropColor = Color3.fromRGB(24, 24, 24),
			MouseEnter = Color3.fromRGB(40, 40, 40),
			StrokeColor = Color3.fromRGB(35, 35, 35),
			HighStrokeColor = Color3.fromRGB(50, 50, 50),
			LineColor = Color3.fromRGB(45, 45, 45),
			SwitchColor = Color3.fromRGB(255, 255, 255),
			TextMuted = Color3.fromRGB(130, 130, 130),
		};
	-- Knox Aurora - Green/teal northern lights aesthetic
	elseif name == "Knox Aurora" then
		KnoxHub.Colors = {
			Highlight = Color3.fromRGB(50, 255, 180),
			Toggle = Color3.fromRGB(80, 200, 255),
			Risky = Color3.fromRGB(255, 150, 100),
			AccentGlow = Color3.fromRGB(60, 255, 200),
			BGDBColor = Color3.fromRGB(6, 12, 10),
			BlockColor = Color3.fromRGB(12, 20, 18),
			BlockBackground = Color3.fromRGB(18, 28, 25),
			DropColor = Color3.fromRGB(22, 35, 32),
			MouseEnter = Color3.fromRGB(40, 65, 58),
			StrokeColor = Color3.fromRGB(35, 55, 50),
			HighStrokeColor = Color3.fromRGB(50, 75, 68),
			LineColor = Color3.fromRGB(45, 70, 62),
			SwitchColor = Color3.fromRGB(255, 255, 255),
			TextMuted = Color3.fromRGB(140, 175, 165),
		};
	...
	KnoxHub:RefreshCurrentColor()
end;

function KnoxHub:RefreshCurrentColor()
	for i,v in next , KnoxHub.Elements.Highlight do
		if v.Element and v.Property then
			v.Element[v.Property] = KnoxHub.Colors.Highlight;
		end;
	end;

	for i,v in next , KnoxHub.Elements do
		if v.Element and v.Property and v.Element:GetAttribute('Enabled') then
			v.Element[v.Property] = KnoxHub.Colors.Highlight;
		end;
	end;

	for i,v in next , KnoxHub.Elements.Risky do
		if v.Element and v.Property then
			v.Element[v.Property] = KnoxHub.Colors.Risky;
		end;
	end;

	for i,v in next , KnoxHub.Elements.BlockColor do
		if v.Element and v.Property then
			v.Element[v.Property] = KnoxHub.Colors.BlockColor;
		end;
	end;

	for i,v in next , KnoxHub.Elements.BGDBColor do
		if v.Element and v.Property then
			v.Element[v.Property] = KnoxHub.Colors.BGDBColor;
		end;
	end;

	for i,v in next , KnoxHub.Elements.StrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = KnoxHub.Colors.StrokeColor;
		end;
	end;

	for i,v in next , KnoxHub.Elements.SwitchColor do
		if v.Element and v.Property and v.Element[v.Property] ~= KnoxHub.Colors.MouseEnter then
			v.Element[v.Property] = KnoxHub.Colors.SwitchColor;
		end;
	end;

	for i,v in next , KnoxHub.Elements.BlockBackground do
		if v.Element and v.Property and v.Element[v.Property] then
			v.Element[v.Property] = KnoxHub.Colors.BlockBackground;
		end;
	end;

	for i,v in next , KnoxHub.Elements.DropColor do
		if v.Element and v.Property then
			v.Element[v.Property] = KnoxHub.Colors.DropColor;
		end;
	end;

	for i,v in next , KnoxHub.Elements.LineColor do
		if v.Element and v.Property then
			v.Element[v.Property] = KnoxHub.Colors.LineColor;
		end;
	end;

	for i,v in next , KnoxHub.Elements.HighStrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = KnoxHub.Colors.HighStrokeColor;
		end;
	end;
end;

function KnoxHub:ChangeHighlightColor(NewColor: Color3)
	local H,S,V = NewColor:ToHSV();

	KnoxHub.Colors.Highlight = NewColor;
	KnoxHub.Colors.Toggle = Color3.fromHSV(H,S,V - 0.2);
...
function KnoxHub.new(Config : Window)

	if not Config.Scale then
		if KnoxHub:_IsMobile() then
			Config.Scale = KnoxHub.Scale.Mobile;
		else
			Config.Scale = KnoxHub.Scale.Window;
		end;
	end;
	...
	Config = KnoxHub.__CONFIG(Config , {
		...
		Keybind = "Insert",
		Logo = KnoxHub.Logo;
		Scale = KnoxHub.Scale.Window,
		TextSize = 15
	});

	local TabHover = KnoxHub.__SIGNAL(false);
	local WindowOpen = KnoxHub.__SIGNAL(true);
	local WindowArgs = {
		SelectedTab = nil,
		Tabs = {},
		LastTab = nil,
	...
	KnoxHub:_DrawKeybinds(KnoxHub);

	UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
		TabButtonScrollingFrame.CanvasSize = UDim2.fromOffset(0,UIListLayout.AbsoluteContentSize.Y)
	end);

	KnoxHub.Name = "u?name=KnoxHub_?"..KnoxHub:_RandomString();
	KnoxHub.Parent = CoreGui;
	KnoxHub.ResetOnSpawn = false
	KnoxHub.IgnoreGuiInset = true;
	KnoxHub.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	KnoxHub.ProtectGui(KnoxHub);

	WindowArgs.Root = KnoxHub;

	table.insert(KnoxHub.Windows , KnoxHub);

	MainFrame.Active = true;
	MainFrame.Name = KnoxHub:_RandomString()
	MainFrame.Parent = KnoxHub
	MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
	MainFrame.BackgroundColor3 = KnoxHub.Colors.BGDBColor
	...
	MainFrame.Size = KnoxHub.Scale.Window
	MainFrame.ZIndex = 4
	...
	KnoxHub:_Animation(MainFrame,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
		Size = Config.Scale
	});
	...
	TabFrame.Name = KnoxHub:_RandomString()
	TabFrame.Parent = MainFrame
	TabFrame.AnchorPoint = Vector2.new(1, 0)
	TabFrame.BackgroundColor3 = KnoxHub.Colors.BGDBColor
	...
	LineFrame1.Name = KnoxHub:_RandomString()
	LineFrame1.Parent = TabFrame
	LineFrame1.AnchorPoint = Vector2.new(1, 0)
	LineFrame1.BackgroundColor3 = KnoxHub.Colors.BGDBColor
	...
	CompLogo.Name = KnoxHub:_RandomString()
	...
	if KnoxHub.CustomHighlightMode then
		CompLogo.ImageColor3 = KnoxHub.Colors.Highlight;
	...
	WindowLabel.Name = KnoxHub:_RandomString()
	...
	WindowLabel.TextColor3 = KnoxHub.Colors.SwitchColor
	...
	TabButtons.Name = KnoxHub:_RandomString()
	...
	SelectionFrame.Name = KnoxHub:_RandomString()
	SelectionFrame.Parent = TabButtons
	SelectionFrame.AnchorPoint = Vector2.new(1, 0)
	SelectionFrame.BackgroundColor3 = KnoxHub.Colors.Highlight
	...
	TabButtonScrollingFrame.Name = KnoxHub:_RandomString()
	...
	Userinfo.Name = KnoxHub:_RandomString()
	...
	do
		...
		Highlight.Name = KnoxHub:_RandomString()
		...
		Userinfo.MouseEnter:Connect(function()
			KnoxHub:_Animation(Highlight,TweenInfo.new(0.2),{
		...
		KnoxHub:_Input(Userinfo,function()
	...
	UserProfile.Name = KnoxHub:_RandomString()
	...
	UserProfile.Image = KnoxHub:CacheImage("rbxassetid://18518299306")

	UICorner_4.CornerRadius = UDim.new(1, 0)
	UICorner_4.Parent = UserProfile

	UserText.Name = KnoxHub:_RandomString()
	...
	UserText.TextColor3 = KnoxHub.Colors.SwitchColor
	...
	ExpireText.Name = KnoxHub:_RandomString()
	...
	ExpireText.TextColor3 = KnoxHub.Colors.SwitchColor
	...
	TabMainFrame.Name = KnoxHub:_RandomString()
	...
	if KnoxHub:_IsMobile() then
		KnoxHub:_AddDragBlacklist(TabButtons);
	end;
	...
	WindowOpen:Connect(function(v)
		...
		if v then
			KnoxHub:_Animation(MainFrame,TweenInfo.new(0.2),{
				Size = Config.Scale
			})

			KnoxHub:_Animation(TabButtonScrollingFrame,TweenInfo.new(0.35),{
				Position = UDim2.new(0.5, 0, 0.5, 0)
			})

			KnoxHub:_Animation(CompLogo,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			KnoxHub:_Animation(WindowLabel,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			KnoxHub:_Animation(UserProfile,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			KnoxHub:_Animation(UserText,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			KnoxHub:_Animation(ExpireText,TweenInfo.new(0.2),{
			...
			KnoxHub:_Animation(LineFrame1,TweenInfo.new(0.3),{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 20, 1, 0)
			})

			KnoxHub:_Animation(TabFrame,TweenInfo.new(0.2),{
			...
			KnoxHub:_Animation(LineFrame1,TweenInfo.new(0.1),{
				BackgroundTransparency = 1,
			...
			KnoxHub:_Animation(TabFrame,TweenInfo.new(0.1),{
				BackgroundTransparency = 1
			})
		end;
	end);
	...
	TabHover:Connect(function(value)
		...
		if value then
			KnoxHub:_Animation(TabFrame , Style , {
				Size = UDim2.new(0, 185,1, 0)
			});

			KnoxHub:_Animation(WindowLabel , Style , {
				Position = UDim2.new(0, 60,0, 17),
				TextTransparency = 0
			});

			KnoxHub:_Animation(UserText , Style , {
				Position = UDim2.new(0, 55,0, 8),
				TextTransparency = 0.1
			});

			KnoxHub:_Animation(ExpireText , Style , {
	...
	do
		...
		UserSettings.Name = KnoxHub:_RandomString()
		UserSettings.Parent = KnoxHub;
		UserSettings.BackgroundColor3 = KnoxHub.Colors.BGDBColor;
		...
		SectionFrame.Name = KnoxHub:_RandomString()
		SectionFrame.Parent = UserSettings
		SectionFrame.BackgroundColor3 = KnoxHub.Colors.BGDBColor;
		...
		Header.Name = KnoxHub:_RandomString()
		...
		HeaderText.Name = KnoxHub:_RandomString()
		...
		ImageLabel.Image = KnoxHub:CacheImage("rbxassetid://10747384394")
		...
		function WindowArgs.UserSettings:Create()
			...
			WindowArgs.UserSettings.Signal = KnoxHub:_Blur(UserSettings,Signal);

			KnoxHub:Drag(UserSettings , UserSettings, 0.15);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				KnoxHub:_Animation(UserSettings,TweenInfo.new(0.2),{
			...
			function WindowArgs.UserSettings:Window(Value)
				if Value then
					...
					KnoxHub:_Animation(SectionFrame,TweenInfo.new(0.2),{
						BackgroundTransparency = 0,
					});

					KnoxHub:_Animation(HeaderText,TweenInfo.new(0.2),{
						TextTransparency = 0,
					});

					KnoxHub:_Animation(ImageLabel,TweenInfo.new(0.2),{
			...
			KnoxHub:_Input(ImageLabel,function()
				WindowArgs.UserSettings:Window(false);
			end);

			WindowArgs.UserSettings:Window(false);

			return KnoxHub:_LoadElement(SectionFrame , true , Signal);
		end;
	end;

	function WindowArgs:SetVisible(bool: boolean)
		KnoxHub.Enabled = bool;
	end;
	...
	function WindowArgs:DrawCategory(config : Category)
		...
		Category.Name = KnoxHub:_RandomString()
		...
		if KnoxHub:_IsMobile() then
			KnoxHub:_AddDragBlacklist(Category);
		end;

		CategoryText.Name = KnoxHub:_RandomString()
		...
		CategoryText.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		TabHover:Connect(function(bool)
			if bool then
				KnoxHub:_Animation(CategoryText,Tween,{
					TextTransparency = 0.500
				});
	...
	function WindowArgs:DrawContainerTab(TabConfig : ContainerTab)
		TabConfig = KnoxHub.__CONFIG(TabConfig,{
			Name = "Tab",
			Icon = "eye",
		});

		local Tween = TweenInfo.new(0.35,Enum.EasingStyle.Quint);
		local TabOpenSignal = KnoxHub.__SIGNAL(false);
		...
		TabButton.Name = KnoxHub:_RandomString()
		...
		if KnoxHub:_IsMobile() then
			KnoxHub:_AddDragBlacklist(TabButton);
		end;

		Icon.Name = KnoxHub:_RandomString()
		Icon.Parent = TabButton
		Icon.AnchorPoint = Vector2.new(0, 0.5)
		Icon.BackgroundColor3 = KnoxHub.Colors.Highlight
		Icon.BackgroundTransparency = 1.000
		Icon.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Icon.BorderSizePixel = 0
		...
		Icon.ZIndex = 3
		Icon.Image = KnoxHub:_GetIcon(TabConfig.Icon);
		Icon.ImageColor3 = KnoxHub.Colors.Highlight
		...
		TabNameLabel.Name = KnoxHub:_RandomString()
		...
		TabNameLabel.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		ContainerTab.Name = KnoxHub:_RandomString()
		...
		Top.Name = KnoxHub:_RandomString()
		...
		Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
			...
			if KnoxHub.PerformanceMode then
				if ContainerTab.Visible then
					KnoxHub:_SetNilP(ContainerTab , TabMainFrame);
				else
					KnoxHub:_SetNilP(ContainerTab , nil);
				end;
			else
				KnoxHub:_SetNilP(ContainerTab , TabMainFrame);
			end;
		...
		local TabOpen = function(bool)
			if bool then
				...
				KnoxHub:_Animation(Icon,Tween,{
					ImageTransparency = 0,
				});

				KnoxHub:_Animation(TabNameLabel,Tween,{
					TextTransparency = 0
				});

				KnoxHub:_Animation(Highlight,Tween,{
					BackgroundTransparency = 0.925
				});
		...
		KnoxHub:_Hover(TabButton,function()
		...
		TabHover:Connect(function(bool)
			if bool then
				KnoxHub:_Animation(TabButton,Tween,{
				...
				KnoxHub:_Animation(UICorner,Tween,{
		...
		KnoxHub:_Input(TabButton,function()
		...
		function TabArgs:DrawTab(TabConfig : TabConfig) -- Internal Tab
			...
			local InternalSignal = KnoxHub.__SIGNAL(false);
			local Frame = Instance.new("Frame")
			local UICorner = Instance.new("UICorner")
			local UIStroke = Instance.new("UIStroke")
			local Highlight = Instance.new("Frame")
			...
			Frame.Parent = Top
			Frame.BackgroundColor3 = KnoxHub.Colors.BlockColor
			...
			Highlight.BackgroundColor3 = KnoxHub.Colors.Highlight
			...
			local ToggleUI = function(bool)
				...
				if bool then
					...
					KnoxHub:_Animation(TextLabel,TweenInfo.new(0.2),{
						TextTransparency = 0
					})
				else
	...
	function WindowArgs:DrawConfig(Configuration : TabConfigManager , Internal)
		Configuration = KnoxHub.__CONFIG(Configuration,{
			Name = "Config",
			Icon = "folder",
			Config = nil
		});
		...
		Icon.Size = UDim2.new(0, 15, 0, 15)
		Icon.ZIndex = 3
		Icon.Image = KnoxHub:_GetIcon(Configuration.Icon);
		Icon.ImageColor3 = KnoxHub.Colors.Highlight
		...
		TabConfig.Name = KnoxHub:_RandomString()
		...
		ConfigList.Name = KnoxHub:_RandomString()
		ConfigList.Parent = TabConfig
		ConfigList.AnchorPoint = Vector2.new(0.5, 0)
		ConfigList.BackgroundColor3 = KnoxHub.Colors.BlockColor
		...
		SectionText.Name = KnoxHub:_RandomString()
		...
		SectionText.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		SectionClose.Name = KnoxHub:_RandomString()
		...
		SectionClose.Image = KnoxHub:CacheImage("rbxassetid://109535175596957")
		...
		UIStroke_2.Color = KnoxHub.Colors.StrokeColor
		...
		Header_2.Name = KnoxHub:_RandomString()
		...
		SectionText_2.Name = KnoxHub:_RandomString()
		...
		SectionText_2.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		SectionClose_2.Name = KnoxHub:_RandomString()
		...
		SectionClose_2.Image = KnoxHub:CacheImage("rbxassetid://109535175596957")
		...
		UIStroke_3.Color = KnoxHub.Colors.StrokeColor
		...
		TextBox.Text = ""
		TextBox.TextColor3 = KnoxHub.Colors.SwitchColor
		...
		Button.BackgroundColor3 = KnoxHub.Colors.SwitchColor
		Button.BackgroundTransparency = 1.000
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		...
		Frame_2.BackgroundColor3 = KnoxHub.Colors.Highlight
		...
		UIStroke_4.Color = KnoxHub.Colors.StrokeColor
		...
		Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
			...
			if KnoxHub.PerformanceMode then
				if TabConfig.Visible then
					KnoxHub:_SetNilP(TabConfig , TabMainFrame);
				else
					KnoxHub:_SetNilP(TabConfig , nil);
				end;
			else
				KnoxHub:_SetNilP(TabConfig , TabMainFrame);
			end;
		...
		local TabOpen = function(bool)
			if bool then
				...
				KnoxHub:_Animation(ConfigList,Tween,{
					BackgroundTransparency = 0,
				});

				KnoxHub:_Animation(AddConfig,Tween,{
					BackgroundTransparency = 0,
				});
				...
				KnoxHub:_Animation(SectionText,Tween,{
					TextTransparency = 0.5
				});
				...
				KnoxHub:_Animation(Frame_2,Tween,{
					BackgroundTransparency = 0.1,
				});
				...
				KnoxHub:_Animation(SectionText_2,Tween,{
					TextTransparency = 0.5
				});

				KnoxHub:_Animation(TextBox,Tween,{
					TextTransparency = 0
				});

				KnoxHub:_Animation(SectionClose,Tween,{
					ImageTransparency = 0.5,
				});

				KnoxHub:_Animation(SectionClose_2,Tween,{
					ImageTransparency = 0.5,
				});
			else
		...
		function TabArgs:_DrawConfig()
			...
			DelButton.Name = KnoxHub:_RandomString()
			...
			DelButton.ZIndex = 14
			DelButton.Image = KnoxHub:CacheImage("rbxassetid://10747362393")
			...
			ConfigBlock.Name = KnoxHub:_RandomString()
			...
			if KnoxHub:_IsMobile() then
				KnoxHub:_AddDragBlacklist(ConfigBlock);
			end;

			ConfigText.Name = KnoxHub:_RandomString()
			...
			ConfigText.TextColor3 = KnoxHub.Colors.SwitchColor
			...
			SaveButton.Name = KnoxHub:_RandomString()
			...
			Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
			Icon.ZIndex = 15
			Icon.Image = KnoxHub:CacheImage("rbxassetid://10734941499")
			Icon.ImageTransparency = 1;

			LoadButton.Name = KnoxHub:_RandomString()
			...
			TextLabel_2.TextColor3 = KnoxHub.Colors.SwitchColor
			...
			Icon_2.Name = KnoxHub:_RandomString()
			...
			Icon_2.ZIndex = 15
			Icon_2.Image = KnoxHub:CacheImage("rbxassetid://10723344270")
			...
			AuthorText.Name = KnoxHub:_RandomString()
			...
			AuthorText.TextColor3 = KnoxHub.Colors.SwitchColor
			...
			function ConfigButton:SetInfo(Author , ConfigName)
				local R,G,B = tostring(math.floor(KnoxHub.Colors.Highlight.R * 255)) , tostring(math.floor(KnoxHub.Colors.Highlight.G * 255)) , tostring(math.floor(KnoxHub.Colors.Highlight.B * 255));
			...
			function ConfigButton:Toggle(v)
				if v then
					KnoxHub:_Animation(ConfigBlock,Tween,{
						BackgroundTransparency = 0
					});

					KnoxHub:_Animation(LinkValues,Tween,{
						Position = UDim2.new(1, -12, 0.5, 0)
					});

					KnoxHub:_Animation(ConfigText,Tween,{
						TextTransparency = 0.3,
					...
					KnoxHub:_Animation(AuthorText,Tween,{
						TextTransparency = 0.5,
						Position = UDim2.new(0,AuthorText:GetAttribute('SPC'), 0.5, 0)
					});

					KnoxHub:_Animation(Icon_2,Tween,{
						ImageTransparency = 0
					});
					...
					KnoxHub:_Animation(TextLabel_2,Tween,{
						TextStrokeTransparency = 0.900,
						TextTransparency = 0
					});
				else
			...
			KnoxHub:_Input(LoadButton,function()
				task.spawn(ConfigButton.OnLoad);
			end);

			KnoxHub:_Input(SaveButton,function()
		...
		function TabArgs:Init()
			...
			KnoxHub:_Input(Button,function()
				if TextBox.Text:byte() then
					WindowArgs.Notify.new({
						Title = "Configs",
						Icon = KnoxHub:_GetIcon(Config.Logo),
			...
			local Refresh = function()
				...
				for i,v in next , FullConfig do
					...
					Button.OnLoad = function()
						WindowArgs.Notify.new({
							Title = "Configs",
							Icon = KnoxHub:CacheImage(Config.Logo),
	...
	function WindowArgs:DrawTab(TabConfig : TabConfig , Internal)
		...
		if Internal then
			...
			TabContent.Name = KnoxHub:_RandomString()
			...
			Internal.Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
				...
				if KnoxHub.PerformanceMode then
					if TabContent.Visible then
						KnoxHub:_SetNilP(TabContent , Internal.Parent);
					else
						KnoxHub:_SetNilP(TabContent , nil);
					end;
				else
					KnoxHub:_SetNilP(TabContent , Internal.Parent);
				end;
			...
			if KnoxHub:_IsMobile() then
				KnoxHub:_AddDragBlacklist(Left);
				KnoxHub:_AddDragBlacklist(Right);
			end;
			...
			Highlight:GetPropertyChangedSignal('BackgroundTransparency'):Connect(function()
				...
				if KnoxHub.PerformanceMode then
					if TabContent.Visible then
						KnoxHub:_SetNilP(TabContent , TabMainFrame);
					else
						KnoxHub:_SetNilP(TabContent , nil);
					end;
				else
		...
		function TabArgs:DrawSection(config: Section)
			...
			Section.Name = KnoxHub:_RandomString()
			...
			Section.BackgroundColor3 = KnoxHub.Colors.BlockColor
			...
			if KnoxHub:_IsMobile() then
				KnoxHub:_AddDragBlacklist(Section);
			end;
			...
			local refresh = function(Upvalue)
				...
				if IsOpen then
					...
					if (Section:GetAttribute('Height') and not KnoxHub:_IsMobile() and FullScale <= RefPos) then
						KnoxHub:_Animation(Section,TweenInfo.new(0.4,Enum.EasingStyle.Quint),{
			...
			TabOpenSignal:Connect(function(bool)
				if bool then
					KnoxHub:_Animation(Section,TweenInfo.new(0.21),{
						BackgroundTransparency = 0
					})

					KnoxHub:_Animation(SectionText,TweenInfo.new(0.21),{
						TextTransparency = 0.500
					})

					KnoxHub:_Animation(SectionClose,TweenInfo.new(0.21),{
						ImageTransparency = 0.500
					})
				else
					KnoxHub:_Animation(Section,TweenInfo.new(0.21),{
			...
			KnoxHub:_Input(Header,function()
				IsOpen = not IsOpen;

				if IsOpen then
					KnoxHub:_Animation(SectionClose,TweenInfo.new(0.35),{
						Rotation = 0
					});
				else
					KnoxHub:_Animation(SectionClose,TweenInfo.new(0.35),{
			...
			Header.MouseEnter:Connect(function()
				KnoxHub:_Animation(SectionText,TweenInfo.new(0.2),{
			...
			return KnoxHub:_LoadElement(Section , true , TabOpenSignal)
		end;
	...
	do
		...
		CloseWindow.Name = KnoxHub:_RandomString()
		CloseWindow.Parent = KnoxHub
		CloseWindow.AnchorPoint = Vector2.new(1, 0)
		CloseWindow.BackgroundColor3 = KnoxHub.Colors.BGDBColor
		...
		local ToggleCloseUI = function(v)
			...
			if v then
				...
				KnoxHub:_Animation(CloseWindow,TweenInfo.new(0.2),{
		...
		function WindowArgs:Watermark()
			local Signal = KnoxHub.__SIGNAL(true);
			...
			Watermark.Name = KnoxHub:_RandomString()
			Watermark.Parent = KnoxHub
			Watermark.AnchorPoint = Vector2.new(1, 0)
			Watermark.BackgroundColor3 = KnoxHub.Colors.BGDBColor

			KnoxHub:Drag(Watermark , Watermark, 0.1);
			...
			Logo.Name = KnoxHub:_RandomString()
			Logo.Parent = Watermark
			Logo.AnchorPoint = Vector2.new(1, 0.5)
			Logo.BackgroundColor3 = KnoxHub.Colors.BGDBColor
			...
			WaternarkList.Name = KnoxHub:_RandomString()
			...
			BackFrame.Name = KnoxHub:_RandomString()
			...
			KnoxHub:_Blur(BackFrame,Signal);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				KnoxHub:_Animation(Watermark,TweenInfo.new(0.4),{
			...
			function Args:AddText(Watermark : Watermark)
				Watermark = KnoxHub.__CONFIG(Watermark, {
					Text = "Watermark",
					Icon = "info"
				});
				...
				Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
				Icon.ZIndex = 156
				Icon.Image = KnoxHub:_GetIcon(Watermark.Icon);
				...
				function Arg:Visible(v)
					...
					if KnoxHub.PerformanceMode then
						if v then
							KnoxHub:_SetNilP(Icon , WaternarkList);
							KnoxHub:_SetNilP(TextLabel , WaternarkList);
						else
							KnoxHub:_SetNilP(Icon , nil);
							KnoxHub:_SetNilP(TextLabel , nil);
						end;
					else
						KnoxHub:_SetNilP(Icon , WaternarkList);
						KnoxHub:_SetNilP(TextLabel , WaternarkList);
		...
		local Button = KnoxHub:_Input(CloseWindow,function()
			WindowArgs:_ToggleUI()
		end)

		if not KnoxHub:_IsMobile() then

			KnoxHub:_Hover(Button,function()
	...
	function WindowArgs:Update(config: WindowUpdate)
		...
		if KnoxHub.SecureMode and string.find(config.UserProfile, "rbxassetid://",1,true) then
			config.UserProfile = KnoxHub:_GetIcon("user");
		end;
	...
	WindowArgs.LOOP_THREAD = task.spawn(function()
		...
		BlurElement.Name = KnoxHub:_RandomString()
		...
		KnoxHub:_Blur(BlurElement , WindowOpen);

		local MovementFrame = Instance.new("Frame")

		MovementFrame.Name = KnoxHub:_RandomString()
		...
		KnoxHub:Drag(MovementFrame,MainFrame,0.1)
		...
		while true do task.wait(0.01);
			...
			if WindowArgs.SelectedTab and WindowArgs.IsOpen then
				...
				if vili < 0 or vili > TabButtons.AbsoluteSize.Y then
					KnoxHub:_Animation(SelectionFrame , TweenInfo.new(0.1) , {
						BackgroundTransparency = 1
					});
				else
					if math.abs(distance) <= 10 then
						...
						KnoxHub:_Animation(SelectionFrame , TweenInfo.new(0.15) , {
							BackgroundTransparency = 0,
	...
	KnoxHub:_Hover(TabFrame , function()
...
function KnoxHub:GetTimeNow(Time)
	Time = Time or tick();
...
function KnoxHub:GetConfig(Type: string)
	local ConfigFlags = {};
...
function KnoxHub:_Path(...)
	local args = {...};

	return table.concat(args, "/");
end;

function KnoxHub:ConfigManager(ConfigManager: ConfigManager) : ConfigFunctions
	ConfigManager = KnoxHub.__CONFIG(ConfigManager , {
		Directory = "KnoxHub",
		Config = "Software"
	});
	...
	if not isfolder(KnoxHub:_Path(ConfigManager.Directory , ConfigManager.Config)) then
		makefolder(KnoxHub:_Path(ConfigManager.Directory , ConfigManager.Config));
	end;

	local Args = {
		Directory = KnoxHub:_Path(ConfigManager.Directory , ConfigManager.Config);
		EnableNotify = false,
	};

	local notify = KnoxHub.newNotify();

	function Args:WriteConfig(Config: WriteConfig)
		Config = KnoxHub.__CONFIG(Config , {
			Name = KnoxHub:_RandomString(),
			Author = LocalPlayer.Name,
		});

		local Flags = KnoxHub:GetConfig("MK");

		Flags["__INFORMATION"] = {
			Type = "Information",
			Author = Config.Author,
			Name = Config.Name,
			CreatedDate = KnoxHub:GetDate()
		};

		if Args.EnableNotify then
			notify.new({
				Title = "Configs",
				Icon = KnoxHub:_GetIcon('settings'),
				Content = "Create config \""..Config.Name.."\""
			})
		end

		writefile(KnoxHub:_Path(Args.Directory , Config.Name) , HttpService:JSONEncode(Flags));
	end;

	function Args:LoadConfigFromString(str: string)
		local decoded = HttpService:JSONDecode(str);

		local Flags = KnoxHub:GetConfig("KV");
	...
	function Args:GetCurrentConfig()
		return KnoxHub:GetConfig("MK")
	end;

	function Args:ReadInfo(ConfigName: string)
		local _path = KnoxHub:_Path(Args.Directory , ConfigName);
...
function KnoxHub:Loader(IconId,Duration)
	local KnoxHub = Instance.new("ScreenGui")

	KnoxHub.Name = KnoxHub:_RandomString()
	KnoxHub.Parent = CoreGui
	...
	Loader.Name = KnoxHub:_RandomString()
	Loader.Parent = KnoxHub
	...
	Icon.ZIndex = 100
	Icon.Image = IconId or KnoxHub.Logo;
	Icon.ImageTransparency = 1

	Vignette.Name = KnoxHub:_RandomString()
	Vignette.Parent = Loader
	Vignette.BackgroundTransparency = 1.000
	Vignette.BorderColor3 = Color3.fromRGB(27, 42, 53)
	Vignette.BorderSizePixel = 0
	Vignette.Size = UDim2.new(1, 0, 1, 0)
	Vignette.Image = KnoxHub:CacheImage("rbxassetid://18720640102")
	Vignette.ImageColor3 = KnoxHub.Colors.Highlight
	Vignette.ImageTransparency = 1
	Vignette.AnchorPoint = Vector2.new(0.5,0.5)
	Vignette.Position = UDim2.fromScale(0.5,0.5)

	KnoxHub:_Animation(Loader,TweenInfo.new(0.55,Enum.EasingStyle.Quint),{
		BackgroundTransparency = 0.5
	});
	...
	task.delay(0.5,function()
		KnoxHub:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint),{
			ImageTransparency = 0.01,
			Size = UDim2.new(0, 200, 0, 200)
		});

		task.delay(0.25,function()
			KnoxHub:_Animation(Vignette,TweenInfo.new(5),{
				ImageTransparency = 0.2
			});

			task.wait(Duration or 4.5)

			KnoxHub:_Animation(Vignette,TweenInfo.new(3,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				Size = UDim2.new(2, 0, 2, 0)
			});

			KnoxHub:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				ImageTransparency = 1,
			});

			KnoxHub:_Animation(Loader,TweenInfo.new(1.5,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				BackgroundTransparency = 1
			});

			task.delay(0.1,function()
				KnoxHub:_Animation(Vignette,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
					ImageTransparency = 1
				});
...
function KnoxHub.newNotify()
	if KnoxHub.NOTIFY_CACHE then
		return KnoxHub.NOTIFY_CACHE;
	end;
	...
	Notification.Name = KnoxHub:_RandomString()
	Notification.Parent = CoreGui;
	Notification.ResetOnSpawn = false
	Notification.ZIndexBehavior = Enum.ZIndexBehavior.Global

	NotifyContainer.Name = KnoxHub:_RandomString()
	...
	KnoxHub.NOTIFY_CACHE = {
		new = function(Notify: Notify) : NotifyPayback
			Notify = KnoxHub.__CONFIG(Notify, {
				Icon = KnoxHub.Logo,
				Title = "Notification",
				Content = "Content",
				Duration = 3,
			...
			BlockFrame.Name = KnoxHub:_RandomString()
			BlockFrame.Parent = NotifyContainer
			BlockFrame.AnchorPoint = Vector2.new(1, 0)
			BlockFrame.BackgroundColor3 = KnoxHub.Colors.BGDBColor
			...
			NotifyFrame.Name = KnoxHub:_RandomString()
			NotifyFrame.Parent = BlockFrame
			NotifyFrame.BackgroundColor3 = KnoxHub.Colors.BGDBColor
			...
			if string.find(Notify.Icon,'cache-ck-',1,true) then	
				...
				CompLogo.Image = KnoxHub:_GetIcon(Notify.Icon);
			end;
			...
			Header.TextColor3 = KnoxHub.Colors.SwitchColor
			Header.TextSize = 14.000
			Header.TextXAlignment = Enum.TextXAlignment.Left

			Body.Name = KnoxHub:_RandomString()
			...
			Body.TextColor3 = KnoxHub.Colors.SwitchColor
			Body.TextSize = 12.000
			Body.TextTransparency = 0.500
			Body.TextXAlignment = Enum.TextXAlignment.Left
			Body.TextYAlignment = Enum.TextYAlignment.Top

			TimeLeftFrame.Name = KnoxHub:_RandomString()
			...
			TimeLeft.Name = KnoxHub:_RandomString()
			TimeLeft.Parent = TimeLeftFrame
			TimeLeft.BackgroundColor3 = KnoxHub.Colors.Highlight
			...
			local UpdateText = function()
				...
				if BlockFrame:GetAttribute('Already') then
					KnoxHub:_Animation(BlockFrame,TweenInfo.new(0.3),{
			...
			local Close = function()
				KnoxHub:_Animation(NotifyFrame,TweenInfo.new(0.65,Enum.EasingStyle.Quint),{
			...
			local Show = function()
				KnoxHub:_Animation(NotifyFrame,TweenInfo.new(0.5,Enum.EasingStyle.Quint),{
					Position = UDim2.new(0,0,0,0)
				});
			end;

			if typeof(Notify.Duration) == 'number' and Notify.Duration ~= math.huge then
				KnoxHub:_Animation(TimeLeft,TweenInfo.new(Notify.Duration + 0.2,Enum.EasingStyle.Linear),{
			...
			return {
				...
				SetProgress = function(self , new , Time)
					...
					KnoxHub:_Animation(TimeLeft,TweenInfo.new(Time or 0.85,(Time and Enum.EasingStyle.Linear) or Enum.EasingStyle.Quint),{
...
KnoxHub.NilFolder.Name = "Nil-Instances";

--[[
    ╔═══════════════════════════════════════════════════════════════╗
    ║              KNOX HUB LOADING SCREEN SYSTEM                   ║
    ╚═══════════════════════════════════════════════════════════════╝
]]--

function KnoxHub:CreateLoadingScreen(Config)
	Config = KnoxHub.__CONFIG(Config or {}, {
		Title = "Knox Hub",
		Subtitle = "Loading...",
		Duration = 3,
		ShowProgress = true,
	...
	LoadingGui.Name = KnoxHub:_RandomString()
	LoadingGui.Parent = CoreGui
	LoadingGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	LoadingGui.ResetOnSpawn = false
	LoadingGui.IgnoreGuiInset = true
	
	KnoxHub.ProtectGui(LoadingGui)
	
	Background.Name = KnoxHub:_RandomString()
	Background.Parent = LoadingGui
	Background.BackgroundColor3 = KnoxHub.Colors.BGDBColor
	Background.BorderSizePixel = 0
	Background.Size = UDim2.new(1, 0, 1, 0)
	Background.ZIndex = 1000
	
	-- Logo Container (centered)
	LogoContainer.Name = KnoxHub:_RandomString()
	...
	-- Logo Glow Effect
	LogoGlow.Name = KnoxHub:_RandomString()
	LogoGlow.Parent = LogoContainer
	LogoGlow.BackgroundTransparency = 1
	LogoGlow.AnchorPoint = Vector2.new(0.5, 0.5)
	LogoGlow.Position = UDim2.new(0.5, 0, 0.5, 0)
	LogoGlow.Size = UDim2.new(1.5, 0, 1.5, 0)
	LogoGlow.ZIndex = 1001
	LogoGlow.Image = KnoxHub.Logo
	LogoGlow.ImageColor3 = KnoxHub.Colors.Highlight
	LogoGlow.ImageTransparency = 0.7
	
	-- Main Logo
	LogoImage.Name = KnoxHub:_RandomString()
	...
	LogoImage.Image = KnoxHub.Logo
	LogoImage.ImageColor3 = Color3.fromRGB(255, 255, 255)
	
	-- Spinner Container (around logo)
	SpinnerContainer.Name = KnoxHub:_RandomString()
	...
	SpinnerContainer.Position = UDim2.new(0.5, 0, 0.4, 0)
	SpinnerContainer.Size = UDim2.new(0, 160, 0, 160)
	SpinnerContainer.ZIndex = 1000
	
	-- Spinner Ring
	Spinner.Name = KnoxHub:_RandomString()
	...
	Spinner.ZIndex = 1000
	Spinner.Image = "rbxassetid://10723433935" -- Loader icon
	Spinner.ImageColor3 = KnoxHub.Colors.Highlight
	Spinner.ImageTransparency = 0.3
	
	-- Spinner Glow
	SpinnerGlow.Name = KnoxHub:_RandomString()
	SpinnerGlow.Parent = SpinnerContainer
	...
	SpinnerGlow.Size = UDim2.new(1.2, 0, 1.2, 0)
	SpinnerGlow.ZIndex = 999
	SpinnerGlow.Image = "rbxassetid://10723433935"
	SpinnerGlow.ImageColor3 = KnoxHub.Colors.Highlight
	SpinnerGlow.ImageTransparency = 0.85
	
	-- Title
	TitleLabel.Name = KnoxHub:_RandomString()
	TitleLabel.Parent = Background
	TitleLabel.BackgroundTransparency = 1
	TitleLabel.AnchorPoint = Vector2.new(0.5, 0)
	TitleLabel.Position = UDim2.new(0.5, 0, 0.58, 0)
	...
	-- Subtitle with typewriter effect support
	SubtitleLabel.Name = KnoxHub:_RandomString()
	SubtitleLabel.Parent = Background
	SubtitleLabel.BackgroundTransparency = 1
	SubtitleLabel.AnchorPoint = Vector2.new(0.5, 0)
	SubtitleLabel.Position = UDim2.new(0.5, 0, 0.64, 0)
	SubtitleLabel.Size = UDim2.new(0, 400, 0, 25)
	SubtitleLabel.ZIndex = 1001
	SubtitleLabel.Font = Enum.Font.Gotham
	SubtitleLabel.Text = Config.Subtitle
	SubtitleLabel.TextColor3 = KnoxHub.Colors.TextMuted or Color3.fromRGB(140, 140, 170)
	SubtitleLabel.TextSize = 16
	SubtitleLabel.TextTransparency = 0.3
	
	-- Progress Container
	ProgressContainer.Name = KnoxHub:_RandomString()
	ProgressContainer.Parent = Background
	ProgressContainer.BackgroundColor3 = KnoxHub.Colors.BlockColor
	ProgressContainer.AnchorPoint = Vector2.new(0.5, 0)
	ProgressContainer.Position = UDim2.new(0.5, 0, 0.72, 0)
	ProgressContainer.Size = UDim2.new(0, 300, 0, 6)
	ProgressContainer.ZIndex = 1001
	ProgressContainer.Visible = Config.ShowProgress
	
	UICorner.CornerRadius = UDim.new(1, 0)
	UICorner.Parent = ProgressContainer
	
	-- Progress Bar with gradient
	ProgressBar.Name = KnoxHub:_RandomString()
	ProgressBar.Parent = ProgressContainer
	ProgressBar.BackgroundColor3 = KnoxHub.Colors.Highlight
	...
	UIGradient.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, KnoxHub.Colors.Toggle),
		ColorSequenceKeypoint.new(1, KnoxHub.Colors.Highlight)
	})
	UIGradient.Parent = ProgressBar
	
	-- Progress Glow
	ProgressGlow.Name = KnoxHub:_RandomString()
	ProgressGlow.Parent = ProgressBar
	ProgressGlow.BackgroundColor3 = KnoxHub.Colors.Highlight
	...
	ProgressGlow.ZIndex = 1001
	
	UICorner_3.CornerRadius = UDim.new(1, 0)
	UICorner_3.Parent = ProgressGlow
	...
	local pulseThread = task.spawn(function()
		while LoadingGui.Parent do
			KnoxHub:_Animation(LogoGlow, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
	...
	local spinThread = task.spawn(function()
		while LoadingGui.Parent do
			...
			SpinnerContainer.Rotation = elapsed * 360 / spinSpeed
			task.wait()
		end
	end)
	...
	if Config.ShowProgress then
		KnoxHub:_Animation(ProgressBar, TweenInfo.new(Config.Duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = UDim2.new(1, 0, 1, 0)
		})
	end
	...
	local function CloseLoading()
		...
		-- Fade out
		KnoxHub:_Animation(Background, TweenInfo.new(0.5), {
			BackgroundTransparency = 1
		})
		KnoxHub:_Animation(LogoImage, TweenInfo.new(0.4), {
			ImageTransparency = 1
		})
		KnoxHub:_Animation(LogoGlow, TweenInfo.new(0.4), {
			ImageTransparency = 1
		})
		KnoxHub:_Animation(Spinner, TweenInfo.new(0.4), {
			ImageTransparency = 1
		})
		KnoxHub:_Animation(SpinnerGlow, TweenInfo.new(0.4), {
			ImageTransparency = 1
		})
		KnoxHub:_Animation(TitleLabel, TweenInfo.new(0.3), {
			TextTransparency = 1
		})
		KnoxHub:_Animation(SubtitleLabel, TweenInfo.new(0.3), {
			TextTransparency = 1
		})
		KnoxHub:_Animation(ProgressContainer, TweenInfo.new(0.3), {
			BackgroundTransparency = 1
		})
		KnoxHub:_Animation(ProgressBar, TweenInfo.new(0.3), {
			BackgroundTransparency = 1
		})
		KnoxHub:_Animation(ProgressGlow, TweenInfo.new(0.3), {
			BackgroundTransparency = 1
		})
	...
	task.delay(Config.Duration, CloseLoading)
...
function KnoxHub:GetThemes()
	return {
		"Knox Neon",      -- Default - Cyan/Purple cyberpunk
		"Knox Ember",     -- Orange/Red fire
		"Knox Frost",     -- Ice blue minimal
		"Knox Phantom",   -- Pure dark monochrome
		"Knox Aurora",    -- Green/Teal northern lights
		-- Legacy themes
		"Dark Green",
		"Dark Blue",
		"Purple Rose",
